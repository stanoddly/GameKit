...
This project verifies clean build, publish, embedded-resource, and ZIP behavior by launching nested `dotnet` processes. Its tests are explicit so a normal `dotnet test GameKit.slnx` cannot run them concurrently with projects that have the shader MSBuild task loaded.
