...
The workflow rejects runs outside the default branch, selects the next `0.0.N-alpha` version, and packs the runtime assemblies and build integrations. Source validation is not repeated because the protected default branch requires successful Windows and ARM test checks before merging. The workflow validates the exact prerelease package through isolated consumers on every supported host, creates the version tag, publishes the NuGet and symbol packages, and creates a GitHub prerelease. `dotnet nuget push` publishes the adjacent `.snupkg` through the configured symbol source.
