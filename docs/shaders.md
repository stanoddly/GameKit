...
The `Pixely` package contains slim build-host tool directories derived from the verified [`stanoddly/slang-dxc-bundle`](https://github.com/stanoddly/slang-dxc-bundle) distributions. Package restore extracts all five directories into NuGet's global package folder. A project with `SdlangShader` items invokes `slangc` directly from its build host's directory there; it does not copy or modify the restored tooling. Consumer builds therefore do not download tooling outside NuGet restore or create a project-local compiler installation.

Projects inside this repository continue to use a `ProjectReference` to `Pixely.SdlangCompiler` plus explicit imports of its props and targets, as shown by the tutorials. A source-tree shader build asks the shared compiler project to download, verify, and extract the complete archive for the current development host under `src/Pixely.SdlangCompiler/obj/`. MSBuild coordinates that preparation target across parallel shader-project builds.

Each packaged platform directory contains `slangc`, the Slang compiler and glslang libraries, the DXC downstream compiler required for DXIL output, the bundle manifest, and the applicable license material. Slang uses the glslang library for SPIR-V optimization even when compiling Slang input, so it cannot be omitted. The package supports Linux x64/ARM64, Windows x64, and macOS x64/ARM64. Host selection is independent of the application's runtime identifier, so cross-publishing still executes the compiler for the machine running the build. DXIL generation is required on every supported host and is never silently omitted. Slang and DXC are build-host tooling and are never copied into application build or publish output. Build integrations pass `$(SlangCompilerPath)` to `SdlangCompiler`.
