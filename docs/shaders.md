...
Projects inside this repository continue to use a `ProjectReference` to `Pixely.SdlangCompiler` plus explicit imports of its props and targets, as shown by the tutorials. Building that shared compiler project downloads and verifies the complete archive for the current development host under `src/Pixely.SdlangCompiler/obj/`.
