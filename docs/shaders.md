...
The `Pixely` package contains slim build-host tool directories derived from pinned [`stanoddly/slang-dxc-bundle`](https://github.com/stanoddly/slang-dxc-bundle) distributions. Package restore extracts all five directories into NuGet's global package folder. A project with `SdlangShader` items invokes `slangc` directly from its build host's directory there; it does not copy or modify the restored tooling. Consumer builds therefore do not download tooling outside NuGet restore or create a project-local compiler installation.

Projects inside this repository continue to use a `ProjectReference` to `Pixely.SdlangCompiler` plus explicit imports of its props and targets, as shown by the tutorials. A source-tree shader build asks the shared compiler project to download and extract the complete archive for the current development host under `src/Pixely.SdlangCompiler/obj/`. MSBuild coordinates that preparation target across parallel shader-project builds.
